...
from .forms import AmbienteForm, SolicitanteForm, GerenciaForm, PuestoForm, VLANForm, ProyectoForm, StatusProyectoForm, PaisForm, REDForm, UsoRedForm, CPUForm, VTIForm, StatusEquipoForm, RAMForm
from .models import AMBIENTE, Bitacora, Perfil, UsuarioExtendido, CLOUD, SOLICITANTE, GERENCIA, PUESTO, VLAN, PROYECTO, STATUS_PROYECTO, PAIS, RED, USO_RED, CONTROL_VLAN, VLAN_IP, CPU, VTI, STATUS_EQUIPO, RAM
...
@custom_login_required
def delete_status_proyecto(request, statuse_id):
    status = get_object_or_404(STATUS_PROYECTO, id_status=statuse_id)
...
def delete_vti(request, vti_id):
    ...
    if request.method == 'POST':
        ...
        registrar_evento(
            ...
            'DELETE',
            'VTI',
            f'Eliminación de VTI: {vti.nombre_vti}'
        )
        return redirect('list_vtis')
    return render(request, 'delete_vti.html', {'vti': vti})

@custom_login_required
def create_ram(request):
    if request.method == 'GET':
        return render(request, 'ram_form.html', {
            'form': RAMForm()
        })
    else:
        try:
            ...
            if form.is_valid():
                ...
                ram.user_create = request.user
                ram.fecha_creacion = timezone.now()
                ram.save()
                ...
                registrar_evento(
                    ...
                    'CREATE',
                    'RAM',
                    f'Creación de RAM: {ram.cantidad}GB'
                )
                
                return redirect('list_rams')
            return render(request, 'ram_form.html', {
                'form': form,
                'error': 'Por favor proporcione datos válidos'
            })
            ...
            return render(request, 'ram_form.html', {
                'form': RAMForm(),
                'error': 'Error al crear la RAM'
            })

@custom_login_required
def list_rams(request):
    ...
    registrar_evento(
        ...
        'VIEW',
        'RAM',
        'Visualización de lista de RAMs'
    )
    
    return render(request, 'ram_list.html', {'rams': rams})

@custom_login_required
def edit_ram(request, ram_id):
    ram = get_object_or_404(RAM, ram_id=ram_id)
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            registrar_evento(
                ...
                'UPDATE',
                'RAM',
                f'Actualización de RAM: {ram.cantidad}GB'
            )
            ...
            return render(request, 'ram_form.html', {
                'form': RAMForm(instance=ram),
                'ram': ram,
                'error': 'Error actualizando la RAM'
            })
...
def delete_ram(request, ram_id):
    ...
    if request.method == 'POST':
        ...
        registrar_evento(
            ...
            'DELETE',
            'RAM',
            f'Eliminación de RAM: {ram.ram_id}'
        )
        return redirect('list_rams')
    return render(request, 'ram_confirm_delete.html', {'ram': ram})

@custom_login_required
def create_status_equipo(request):
    if request.method == 'GET':
        return render(request, 'status_equipo_form.html', {
            'form': StatusEquipoForm()
        })
    else:
        try:
            form = StatusEquipoForm(request.POST)
            if form.is_valid():
                nuevo_status = form.save(commit=False)
                
                # Encontrar el ID más pequeño disponible
                ids_existentes = set(STATUS_EQUIPO.objects.values_list('statuse_id', flat=True))
                id_nuevo = 1
                while id_nuevo in ids_existentes:
                    id_nuevo += 1
                
                nuevo_status.statuse_id = id_nuevo
                nuevo_status.user_create = request.user
                nuevo_status.fecha_creacion = timezone.now()
                nuevo_status.save()
                ...
                registrar_evento(
                    ...
                    'CREATE',
                    'STATUS_EQUIPO',
                    f'Creación de estado de equipo: {nuevo_status.nombre_statuse}'
                )
                
                messages.success(request, 'Estado de equipo creado exitosamente.')
                return redirect('list_status_equipo')
            else:
                return render(request, 'status_equipo_form.html', {
                    'form': form,
                    'error': 'Por favor verifique los datos ingresados'
            ...
            return render(request, 'status_equipo_form.html', {
                'form': StatusEquipoForm(),
                'error': f'Error creando el estado de equipo: {str(e)}'
            })

@custom_login_required
def list_status_equipo(request):
    status_equipo = STATUS_EQUIPO.objects.all()
    ...
    registrar_evento(
        ...
        'VIEW',
        'STATUS_EQUIPO',
        'Visualización de lista de estados de equipo'
    )
    
    return render(request, 'list_status_equipo.html', {'status_equipo': status_equipo})

@custom_login_required
def edit_status_equipo(request, statuse_id):
    status = get_object_or_404(STATUS_EQUIPO, statuse_id=statuse_id)
    
    if request.method == 'GET':
        form = StatusEquipoForm(instance=status)
        return render(request, 'edit_status_equipo.html', {
            'status': status,
            'form': form
        })
    else:
        try:
            form = StatusEquipoForm(request.POST, instance=status)
            if form.is_valid():
                form.save()
                ...
                registrar_evento(
                    ...
                    'UPDATE',
                    'STATUS_EQUIPO',
                    f'Actualización de estado de equipo: {status.nombre_statuse}'
                )
                
                messages.success(request, 'Estado de equipo actualizado exitosamente.')
                return redirect('list_status_equipo')
            else:
                return render(request, 'edit_status_equipo.html', {
                    'status': status,
            ...
            return render(request, 'edit_status_equipo.html', {
                'status': status,
                'form': form,
                'error': f'Error actualizando el estado de equipo: {str(e)}'
            })

@custom_login_required
def delete_status_equipo(request, statuse_id):
    status = get_object_or_404(STATUS_EQUIPO, statuse_id=statuse_id)
    nombre_status = status.nombre_statuse
    status.delete()
    ...
    registrar_evento(
        ...
        'DELETE',
        'STATUS_EQUIPO',
        f'Eliminación de estado de equipo: {nombre_status}'
    )
    
    messages.success(request, 'Estado de equipo eliminado exitosamente.')
    return redirect('list_status_equipo')
