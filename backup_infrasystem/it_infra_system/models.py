...
class STATUS_EQUIPO(models.Model):
    statuse_id = models.AutoField(primary_key=True)
    nombre_statuse = models.CharField(max_length=100, blank=True, null=True)
    descripcion_statuse = models.TextField(blank=True, null=True)
    user_create = models.ForeignKey(User, on_delete=models.CASCADE)
    fecha_creacion = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return self.nombre_statuse if self.nombre_statuse else f"Estado {self.statuse_id}"
...
class RAM(models.Model):
    ...
    vti = models.ForeignKey(VTI, on_delete=models.SET_NULL, null=True, blank=True, related_name='rams')
    ...
    def __str__(self):
        return f"RAM {self.ram_id} - {self.cantidad}GB"      